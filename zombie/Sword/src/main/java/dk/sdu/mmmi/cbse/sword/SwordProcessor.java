...
public class SwordProcessor implements IEntityProcessingService    {
 private static List<Entity> swords = new ArrayList<>();
    @Override
    public void process(GameData gd, World world) {
        
          for (Entity sword: swords) {
            WeaponPart weaponPart = (WeaponPart) world.getMapByPart(WeaponPart.class.getSimpleName()).get(sword.getUUID());
            PositionPart weaponPosition = (PositionPart) world.getMapByPart(PositionPart.class.getSimpleName()).get(sword.getUUID());
            ...
            if (weaponPart.isIsAttacking()) {
                ...
                world.addtoEntityPartMap(new VisualPart("Sword", 10, 10), bullet);
        ...
        protected static void addToProcessingList(Entity sword) {
