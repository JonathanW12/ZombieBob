                    ...
                    for (Map.Entry<UUID, EntityPart> entry2 : world.getMapByPart("ColliderPart").entrySet()) {
                        ...
                        if (structure1 == null || structure2 == null) {
                            ...
                            if (entry.getKey() != entry2.getKey()
                                    && !collider2.getCollidingEntities().contains(entry.getKey())
                                    && !collider1.getCollidingEntities().contains(entry2.getKey())) {

                                if (collider1.getRadius() != 0 && collider2.getRadius() != 0) {
                                    if (collider1Pos != null && collider2Pos != null) {
                                        float distance = (float) Math.sqrt(
                                                Math.pow(collider1Pos.getX() - collider2Pos.getX(), 2)
                                                + Math.pow(collider1Pos.getY() - collider2Pos.getY(), 2));
